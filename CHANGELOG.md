...
- Feat (AL): `global_id` (already computed on every AL node for cross-graph federation) is now reachable through MCP. `bcatlas_get_node` includes it in its output, and a new `bcatlas_find_by_global_id` tool resolves a `global_id` copied from one graph to the matching node(s) on another — the missing resolver half of the federation join (bc-code-atlas#26).
- Feat (AL): edges can now be filtered to only those crossing an app/extension boundary. Every AL node is stamped with `al_owning_app` (the app.json publisher+name that owns the file it came from, independent of AL namespace — two apps can share a namespace convention, so this can't reuse the existing namespace-first federation qualifier). `bcatlas_query_graph`'s `context_filter` accepts a new `cross_app` value (structural, not a relation kind — ANDs with any relation-kind filters given alongside it) that keeps only edges whose two endpoints belong to different apps (bc-code-atlas#27).
- Fix (AL): object node labels now include the object type and numeric ID, not just the bare name. `_extract_generic`'s class-name resolution falls through to `name_fallback_child_types` for AL objects (whose name lives on the `object_name` grammar field, not `name`), which grabbed only the first `quoted_identifier`/`identifier` child — the bare name (e.g. `"Gen. Jnl.-Post Line"`). The object type keyword and numeric ID (both real, separate grammar fields — `object_id`/`object_name`, confirmed via the live AST) were dropped from the label entirely, even though `object_type` was already captured separately for `global_id`. This meant a query using AL's own canonical object reference format (`Codeunit 12 "Gen. Jnl.-Post Line"`) never matched anything, and two same-named objects of different types (e.g. a table and a page both named `"Item"`) were indistinguishable by label. Labels are now `{Type} {Id} "{Name}"` (or `{Type} "{Name}"` for kinds with no numeric ID — interface, controladdin). The bare name still resolves via substring matching (unaffected, no regression), and node IDs/edges are untouched — this only corrects the display label, computed in a later, AL-specific post-process pass than the one that determines node identity. A new `al_object_name` field carries the bare name separately for anything that needs it (e.g. cross-file name-based fact resolution, which previously derived the bare name by parsing the label and needed updating to use this instead).
- Feat (AL): control add-in `event(...)` declarations and page `usercontrol` usage are no longer invisible. `event_declaration` was absent from the AL function types, so a controladdin's events (the JS->AL contract) were silently dropped and no handler could be traced to its event; they are now emitted as `.OnFoo()` nodes parented to the add-in, exactly like its procedures. A page `usercontrol(<ctrl>; <AddIn>)` now emits a `usercontrol` edge (page object -> add-in), and `CurrPage.<ctrl>.<proc>()` calls resolve through the page's control->add-in map to a cross-file `calls` edge on the add-in's procedure — so an add-in is no longer called-by-nobody across files (#41).
