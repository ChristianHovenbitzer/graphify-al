...
_ROUTING_SCHEMA_PROPERTIES: dict = {
    ...
    "project_path": {
        ...
        "description": (
            ...
            " this server was started with. Mutually exclusive with"
            " `country`/`version`."
        ),
    },
}
...
def _subgraph_to_text(
    ...
    *,
    ...
    scores: optional query-relevance map (see _rank_scores / _score_nodes).
    Hop distance from the seeds is always recomputed internally (BFS over
    both edge directions); scores breaks ties among nodes at the same hop
    distance before falling back to raw degree, so a node that actually
    matches the query outranks an unrelated hub with a higher raw degree,
    which pure degree-sorting got backwards (a directly relevant answer
    could be buried behind hundreds of high-connectivity nodes and cut off
    by token_budget before ever being rendered).
    """
    ...
    #
    # scores (query-relevance, see _rank_scores/_score_nodes) breaks ties among
    # nodes at the same hop distance before falling back to raw degree, so an
    # unrelated high-degree hub can't outrank a lower-degree node that actually
    ...
    ordered = seed_hits + sorted(
        nodes - seed_set,
        key=lambda n: (dist.get(n, 1 << 30), -scores.get(n, 0.0), -G.degree(n), str(n)),
    )
    for nid in ordered:
        d = G.nodes[nid]
...
def _build_server(
    ...
    *,
    ...
    # of project_path graphs (upstream's generic multi-project cache). This
    # preserves the configured graph's warm index while preventing a shared
    # server from retaining every project it serves. Used only by the
    # `project_path` branch of `_resolve_ctx` below -- the zero-argument
    # default path still reads `G`/`communities` directly, as before.
    ...
    # Active per-request context, tracked for the handlers below that key off
    # a plain path (report/labels lookups) rather than a full `_RoutedGraph`.
    # Fail-fast at startup on a missing/corrupt default graph (`_load_ctx` ->
    # `_load_graph` raises), matching this fork's existing CLI/startup
    # semantics -- unlike upstream, AL's handlers never defensively check for
    # a `None` G, so silently degrading to a pure multi-project server here
    # would risk a deep AttributeError instead of a clean startup failure.
    active_graph_path = _default_graph_path
    G, communities = _load_ctx(_default_graph_path)

    def _maybe_reload() -> None:
        """Refresh the default graph in place if graph_path's mtime/size changed.

        Delegates to `_ctx_cache`'s own stat+key staleness check (cheap: skips
        the reload entirely when unchanged), so this fork no longer needs its
        own separate reload-lock/mtime-state bookkeeping for the default graph.
        Also resets `active_graph_path` to the default -- resources (which have
        no per-call routing arguments) always want the default graph, and a
        routed tool call's own `_resolve_ctx` call overwrites this correctly
        right after, so resetting it here first is harmless for tool calls.
        """
    ...
    def _resolve_ctx(arguments: dict) -> _RoutedGraph:
        ...
        None of `country`/`version`/`project_path` supplied -> this server's
        own default graph (hot-reloaded via `_ctx_cache`'s pinned entry, see
        `_maybe_reload`). `country`+`version` supplied together -> that
        specific routed (country, version) pair's graph, immutable once
        built (cached; see `_load_routed_graph`). `project_path` alone ->
        a different project's graph.json (cached, mtime-checked; see
        `_GraphContextCache`). At most one routing scheme per call --
        combining `project_path` with `country`/`version` is a caller error,
        and supplying only one of `country`/`version` is too.
        """
        nonlocal active_graph_path
        country = arguments.get("country")
        version = arguments.get("version")
        project_path = arguments.get("project_path")
        if project_path and (country or version):
            raise _RoutingError(
                "`project_path` cannot be combined with `country`/`version` --"
                " pick one routing scheme per call."
            )
        if project_path:
            resolved_path = _resolve_graph_path(project_path)
            g, comm = _load_ctx(resolved_path)
            active_graph_path = resolved_path
            return _RoutedGraph(
                G=g, communities=comm,
                source_root=Path(project_path).resolve(), graph_path=resolved_path,
            )
        if country is None and version is None:
            g, comm = _load_ctx(_default_graph_path)
            active_graph_path = _default_graph_path
            return _RoutedGraph(
                G=g, communities=comm, source_root=_source_root, graph_path=graph_path
            )
        ...
        routed = _load_routed_graph(country, version, data_root)
        active_graph_path = routed.graph_path
        return routed
    ...
    def _ambiguity_message(G: nx.Graph, rivals: list[str]) -> str:
        # FR-005 (spec 004-al-object-labels): a query matching more than one
        # node in the winning tier, across different source files, is
        # genuinely ambiguous (e.g. a table and a page both named "Item")
        # -- list the candidates by their full (now type+ID-qualified)
        # label so the caller can retry unambiguously, rather than
        # silently taking matches[0]. Uses find_node_ambiguity rather than
        # a raw tier count so several matches within one file (a file node
        # plus its members) are not mistaken for ambiguity.
        candidates = "\n".join(
    ...
    def _tool_get_node(arguments: dict) -> str:
        ...
        rivals = find_node_ambiguity(ctx.G, label)
        if len(rivals) > 1:
    ...
    def _tool_shortest_path(arguments: dict) -> str:
        try:
            ...
            return str(exc)
        return _shortest_path_text(ctx.G, arguments)
    ...
    async def read_resource(uri: AnyUrl) -> str:
        _maybe_reload()  # resources read the server's default graph
        uri_str = str(uri)
    ...
    async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
        # Keeps the default graph fresh before every call; a routed call
        # (country/version/project_path) resolves its own graph inside the
        # handler via _resolve_ctx, which is unaffected by this.
        _maybe_reload()
